...
### Black Setup
We use `black` to format python code. Install the `Black Formatter` VSCode extension and `pip install black` in the virtual environment you use for development. From the root directory,
```
black backend
```
will format all of the python code.
...
```
nvm exec 14 npx ros-typescript-generator --config ./ros-ts-generator-config.json
```
Be sure to switch back to node v20 (`nvm use 20`), if needed, after generating the code. This only has to be done if the message/service definitions in `r2c_interfaces` changes.
