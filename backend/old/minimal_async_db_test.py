...
async def setup_minimal_test_db():
    ...
    try:
        ...
        conn = await asyncpg.connect(DEFAULT_POSTGRES_URL, loop=asyncio.get_running_loop())
        ...
        logger.info(f"Minimal Test Database '{db_name}' ensured (dropped and recreated).")
...
async def minimal_fastapi_app(setup_minimal_test_db) -> AsyncGenerator[FastAPI, None]:
    ...
    including a simple health check route and a lifespan context manager.
    It now initializes an actual DB pool in app.state.
    """
    logger.info("Initializing minimal FastAPI app with /health endpoint and lifespan (DB pool initialized).")

    # Define the lifespan context manager for the FastAPI app
    @asynccontextmanager
    async def lifespan_context(app: FastAPI):
        logger.info("FastAPI app startup event: Lifespan STARTED.")
        # NEW: Initialize the local_db_pool with an actual asyncpg pool
        ...
        app.state.local_db_pool = await asyncpg.create_pool(
            ...
            # No explicit loop argument here
        )
        ...
        try:
            ...
            # NEW: Close the database connection pool on application shutdown
    ...
    async def health_check():
        ...
        return {"status": "ok", "db_pool_initialized": hasattr(app.state, 'local_db_pool') and app.state.local_db_pool is not None}
...
async def minimal_test_client(minimal_fastapi_app: FastAPI) -> AsyncGenerator[AsyncClient, None]:
    """
    ...
    async with AsyncClient(transport=ASGITransport(app=minimal_fastapi_app), base_url="http://test") as client:
        yield client
...
async def test_minimal_app_startup(
    ...
    """
    logger.info("Running test_minimal_app_startup (checking /health endpoint and DB pool initialization)...")
    try:
        ...
        # CHANGED: Assert that db_pool_initialized is True, as the pool is now initialized
        assert response.json() == {"status": "ok", "db_pool_initialized": True}, f"Expected {{'status': 'ok', 'db_pool_initialized': True}}, but got {response.json()}"
        logger.info("SUCCESS: Minimal FastAPI app /health endpoint responded correctly and reported DB pool initialized.")

        # No direct DB pool functional test yet in this step

        logger.info("SUCCESS: All minimal FastAPI app startup tests passed.")
    except Exception as e:
