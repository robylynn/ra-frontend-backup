...
async def setup_test_database_and_app():
    ...
    os.environ["DEPLOYMENT_ENV"] = (
        "testing"  # Temporarily set to testing for db_manager
    )
    ...
    try:
        print(
            f"INFO: Running db_manager.py --action auto_setup --drop-local-existing for test DB '{db_name}'..."
        )
    ...
    try:
        ...
        )  # Recreate for next session if needed, or simply drop
        print(
            f"INFO: Test database '{db_name}' dropped and recreated (for clean session start)."
        )
    except Exception as e:
...
@pytest_asyncio.fixture(
    scope="function"
)  # Use function scope for client to ensure clean state per test
async def client(
    backend_api: FastAPI,
) -> AsyncGenerator[AsyncClient, None]:  # Depends on backend_api fixture
    """
    ...
    async with AsyncClient(
        app=backend_api, base_url="http://test"
    ) as ac:  # Use backend_api fixture
        yield ac
...
async def clear_tables(db_pool: AsyncpgPool):
    ...
    tables_to_clear = list(LOADED_RAW_SCHEMA.tables.keys()) + [
        "frontend_configs"
    ]  # Ensure frontend_configs is included
    ...
    async with db_pool.acquire() as conn:
        for table_name in tables_to_clear:
            try:
                ...
                print(
                    f"WARNING: Table '{table_name}' not found during TRUNCATE. Skipping."
                )
            except Exception as e:
