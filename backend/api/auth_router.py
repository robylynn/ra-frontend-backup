...
from api.schema_models import LOADED_RAW_SCHEMA # To check if 'users' table is in schema
...
async def get_user(db_pool: AsyncpgPool, username: str) -> Optional[UserInDB]:
    ...
    async with db_pool.acquire() as conn:
        record = await conn.fetchrow("SELECT user_id, username, hashed_password, created_at FROM users WHERE username = $1;", username)
        if record:
...
async def create_new_user(db_pool: AsyncpgPool, user: UserCreate) -> User:
    ...
    try:
        async with db_pool.acquire() as conn:
            record = await conn.fetchrow(insert_sql, new_user_id, user.username, hashed_password, created_at)
            if record:
                return User(**record)
...
async def authenticate_user(db_pool: AsyncpgPool, username: str, password: str) -> Optional[UserInDB]:
...
async def get_current_user(token: Optional[str] = Depends(oauth2_scheme), db_pool: AsyncpgPool = Depends(get_db_connection)) -> Optional[User]:
    """
    ...
    return User(user_id=user_in_db.user_id, username=user_in_db.username, created_at=user_in_db.created_at)


# --- API Endpoints ---

@auth_router.post("/register", summary="Register a new user", response_model=ApiResponse)
async def register_user(
    ...
    if request.method == "OPTIONS":
        logger.info("Auth Router: Received OPTIONS request for /register. Returning 200 OK.")
        return Response(status_code=status.HTTP_200_OK)

    logger.info(f"Auth Router: /register endpoint hit. Method: {request.method}. Attempting to register new user: {user_create.username}")
    ...
    return ApiResponse(
        ...
        data=new_user.model_dump(exclude_unset=True) if hasattr(new_user, 'model_dump') else new_user.dict(exclude_unset=True)
    )

@auth_router.post("/token", summary="Generate JWT Access Token", response_model=ApiResponse)
async def login_for_access_token(
    ...
    return ApiResponse(
        ...
        data={
            ...
            "token_type": "bearer",
            "user": user.model_dump(exclude={"hashed_password"}) if hasattr(user, 'model_dump') else user.dict(exclude={"hashed_password"})
        }
    )

@auth_router.get("/users/me", summary="Get current authenticated user", response_model=ApiResponse)
async def read_users_me(current_user: User = Depends(get_current_user)):
    ...
    return ApiResponse(
        ...
        data=current_user.model_dump(exclude_unset=True) if hasattr(current_user, 'model_dump') else current_user.dict(exclude_unset=True)
    )

@auth_router.post("/logout", summary="Logout user", response_model=ApiResponse)
async def logout(response: Response, current_user: Optional[User] = Depends(get_current_user)):
    """
    ...
    if current_user:
        ...
        logger.info("Attempting to log out unauthenticated user (clearing potential stale cookie).")
