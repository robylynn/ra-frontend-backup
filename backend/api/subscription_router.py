...
class Connection:
    ...
    def __init__(self, websocket: WebSocket, client_id: str):
        ...
        self.subscriptions: Set[str] = set() # Set of table_names this client is subscribed to
...
class SubscriptionManager:
    ...
    def __init__(self):
        ...
        self.table_subscribers: Dict[str, Set[str]] = {} # {table_name: {client_id, ...}}
    ...
    async def disconnect(self, client_id: str):
        ...
        if client_id in self.active_connections:
            ...
            logger.info(f"Client {client_id} disconnected. Remaining active connections: {len(self.active_connections)}")

    async def subscribe(self, client_id: str, table_name: str, send_historical: bool = False, historical_limit: int = 100):
        ...
        if table_name not in LOADED_RAW_SCHEMA.tables:
            logger.warning(f"Client {client_id}: Attempted to subscribe to unknown table '{table_name}'.")
            await connection.send_json({"type": "error", "message": f"Table '{table_name}' not found or not defined in schema."})
            return
        ...
        if send_historical:
            logger.info(f"Client {client_id}: Fetching historical data for {table_name} (limit={historical_limit})...")
            local_db_pool = connection.websocket.app.state.local_db_pool # Access pool from app state
            ...
            await connection.send_json(
                {"type": "historical", "table": table_name, "data": serializable_historical_data}
            )
            logger.info(f"Client {client_id}: Sent {len(serializable_historical_data)} historical records for {table_name}.")

        await connection.send_json({"type": "status", "table": table_name, "status": "subscribed"})
    ...
    async def unsubscribe(self, client_id: str, table_name: str):
        ...
        if not self.table_subscribers[table_name]:
            del self.table_subscribers[table_name] # Clean up empty sets
        logger.info(f"Client {client_id} successfully unsubscribed from '{table_name}'.")
        await connection.send_json({"type": "status", "table": table_name, "status": "unsubscribed"})
    ...
    async def publish(self, table_name: str, message: Dict[str, Any]):
        ...
        if table_name in self.table_subscribers:
            for client_id in list(self.table_subscribers[table_name]): # Iterate over a copy to avoid issues if clients disconnect
                connection = self.active_connections.get(client_id)
                if connection:
                    try:
                        await connection.queue.put(message)
                        logger.debug(f"Published message for '{table_name}' to client {client_id}'s queue.")
                    except Exception as e:
                        logger.error(f"Error publishing to client {client_id} for table '{table_name}': {e}")
                        # Consider removing client if their queue causes consistent errors
                else:
                    logger.warning(f"Client {client_id} for table '{table_name}' not found in active connections during publish. Cleaning up.")
...
async def websocket_zmq_stream(websocket: WebSocket):
    ...
    try:
        ...
        async def send_to_client_task():
            try:
                while True:
                    ...
                    logger.debug(f"Client {client_id} (send_task): Sending message: {message['type']} for {message.get('table', 'N/A')}")
                    await connection.send_json(message)
                    connection.queue.task_done()
            except asyncio.CancelledError:
                logger.info(f"Client {client_id} (send_task) cancelled.")
            except WebSocketDisconnect:
                logger.info(f"Client {client_id} (send_task) detected client disconnect.")
            ...
            try:
                while True:
                    ...
                    if message_type == "subscribe" and table_name:
                        ...
                        await subscription_manager.subscribe(client_id, table_name, send_historical, historical_limit)
                    elif message_type == "unsubscribe" and table_name:
                        await subscription_manager.unsubscribe(client_id, table_name)
                    else:
                        logger.warning(f"Client {client_id}: Received unknown or invalid message: {message}")
                        await connection.send_json({"type": "error", "message": "Invalid command."})
            except WebSocketDisconnect:
                logger.info(f"Client {client_id} (receive_task) detected client disconnect.")
                raise # Re-raise to main try-except
            except asyncio.CancelledError:
        ...
        logger.info(f"Client {client_id} disconnected from /ws/zmq_stream gracefully.")
    except Exception as e:
        logger.error(f"Unhandled error for client {client_id} on /ws/zmq_stream: {e}")
    finally:
        if send_task:
            send_task.cancel()
        if receive_task:
        ...
        try:
            if send_task: await send_task
            if receive_task: await receive_task
        except asyncio.CancelledError:
