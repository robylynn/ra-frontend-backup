...
@ui_router.get("/config/{client_id}", summary="Get Frontend Configuration for Client", response_model=ApiResponse)
async def get_frontend_config(client_id: str, request: Request):
    ...
    """
    logger.info(f"Attempting to retrieve frontend configuration for client: '{client_id}'.")
    ...
    if LOADED_RAW_SCHEMA is None or table_name not in LOADED_RAW_SCHEMA.tables:
        logger.critical(f"Frontend config table '{table_name}' not defined in schema.yml. Cannot retrieve config.")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ApiResponse(success=False, message=f"Internal server error: '{table_name}' table schema not found. Ensure schema.yml is updated.").model_dump()
        )
    ...
    try:
        ...
        if record:
            logger.info(f"Successfully retrieved frontend configuration for client '{client_id}'.")
            return ApiResponse(
                success=True,
            ...
            logger.info(f"No frontend configuration found for client '{client_id}'. Returning 404.")
            # Correctly raise HTTPException for 404, let FastAPI handle it
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=ApiResponse(success=False, message=f"No configuration found for client '{client_id}'.").model_dump()
            )
    except HTTPException:
        # Re-raise HTTPExceptions directly, don't wrap them in a 500
        raise
    except Exception as e:
        logger.error(f"Unexpected error retrieving frontend configuration for client '{client_id}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ApiResponse(success=False, message=f"An unexpected error occurred: {e}").model_dump()
        )


@ui_router.post("/config/{client_id}", summary="Save Frontend Configuration for Client", status_code=status.HTTP_200_OK, response_model=ApiResponse)
async def save_frontend_config(client_id: str, request: Request, config_payload: FrontendConfigData = Body(...)):
    """
    ...
    The configuration is stored in a 'frontend_configs' table.
    """
    logger.info(f"Attempting to save/update frontend configuration for client: '{client_id}'.")
    local_db_pool: AsyncpgPool = request.app.state.local_db_pool
    table_name = "frontend_configs" # Hardcoded table name for UI configurations

    if LOADED_RAW_SCHEMA is None or table_name not in LOADED_RAW_SCHEMA.tables:
        logger.critical(f"Frontend config table '{table_name}' not defined in schema.yml. Cannot save config.")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ApiResponse(success=False, message=f"Internal server error: '{table_name}' table schema not found. Ensure schema.yml is updated.").model_dump()
        )
    ...
    try:
        ...
        await local_db_pool.execute(upsert_sql, client_id, json_data_to_store, current_time) # Pass serialized string
        
        logger.info(f"Successfully saved/updated frontend configuration for client '{client_id}'.")
        return ApiResponse(
            success=True,
        ...
        logger.error(f"Validation error saving frontend config for client '{client_id}': {e.errors()}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=ApiResponse(success=False, message="Invalid configuration data provided.", data={"errors": e.errors()}).model_dump()
        )
    except Exception as e:
        # NEW: Convert config_payload.config_json to string for logging to avoid f-string error
        logger.error(f"Unexpected error saving frontend configuration for client '{client_id}' with payload {str(config_payload.config_json)}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ApiResponse(success=False, message=f"An unexpected error occurred: {e}").model_dump()
        )
