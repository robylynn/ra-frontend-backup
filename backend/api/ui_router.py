...
async def _get_config(
    table_name: str, client_id: str, model: T, local_db_pool: AsyncpgPool
) -> T:
    logger.info(
...
async def _save_config(
    ...
    logger.info(f"Attempting to save/update configuration to table: '{table_name}'.")
...
async def save_opc_config(
    client_id: str,
    request: Request,
    config_payload: OpcConfiguration = Body(...),
...
async def get_full_config(
    ...
    try:
        ui_config = await _get_config(
            table_name="frontend_configs",
            client_id=client_id,
            model=UiConfiguration,
            local_db_pool=local_db_pool,
        )
        ...
        if e.status_code == 404:
            logger.warning(
                f"No UI configuration found for client {client_id}, using default."
            )
        else:
            raise

    try:
        io_config = await _get_config(
            table_name="io_configs",
            client_id=client_id,
            model=IoConfiguration,
            local_db_pool=local_db_pool,
        )
        ...
        if e.status_code == 404:
            logger.warning(
                f"No IO configuration found for client {client_id}, using default."
            )
        else:
            raise

    try:
        opc_config = await _get_config(
            table_name="opc_configs",
            client_id=client_id,
            model=OpcConfiguration,
            local_db_pool=local_db_pool,
        )
        ...
        if e.status_code == 404:
            logger.warning(
                f"No OPC configuration found for client {client_id}, using default."
            )
        else:
            raise
