...
async def _save_config(table_name: str, client_id: str, config_data: ConfigData, local_db_pool: AsyncpgPool):
    logger.info(
    ...
    try:
        ...
        current_time = datetime.now(timezone.utc)
        # NEW: Serialize the config_payload.config_json (which is a dict) to a JSON string
        json_data_to_store = json.dumps(config_data.config_json)
        await local_db_pool.execute(
            upsert_sql, client_id, json_data_to_store, current_time
        )  # Pass serialized string
        ...
        return ApiResponse(
            ...
            data={
                "client_id": client_id,
                "config_data_saved": config_data.config_json,
                "last_updated": current_time.isoformat().replace("+00:00", "Z"),
            },
        )
    except ValidationError as e:
        ...
        # NEW: Convert config_payload.config_json to string for logging to avoid f-string error
        logger.error(
            f"Unexpected error saving frontend configuration for client '{client_id}' with payload {str(config_data.config_json)}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def save_frontend_config(
    client_id: str, request: Request, config_payload: ConfigData = Body(...)
):
    """
    Saves or updates the UI configuration for a specific frontend client.
    This allows users to persist their custom dashboard/plot settings.
    ...
    local_db_pool: AsyncpgPool = request.app.state.local_db_pool
    return _save_config("frontend_configs", client_id=client_id, config_data=config_payload, local_db_pool=local_db_pool)
...
async def save_io_config(
    client_id: str, request: Request, config_payload: io_configuration_models.IoConfiguration = Body(...)
):
    logger.info(
        f"Attempting to save/update IO configuration for client: '{client_id}'."
    )
    local_db_pool: AsyncpgPool = request.app.state.local_db_pool
    return _save_config("io_configs", client_id=client_id, config_data=config_payload, local_db_pool=local_db_pool)
