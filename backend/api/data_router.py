...
from backend.api.models import ApiState, typedAppState
from backend.api.schema_models import LOADED_RAW_SCHEMA
from common_models.autogenerated.system_models import ApiResponse
from common_models.autogenerated.database_models import AutogeneratedPydanticModel, DYNAMIC_TABLE_MODELS
...
async def create_table(table_name: str, request: Request, state: ApiState = Depends(typedAppState)):
    """
    ...
    try:
        ...
        async with local_db_pool.acquire() as conn:
            ...
            if hypertable_column:
                ...
                logger.info(
                    f"Table '{table_name}' created successfully on local DB."
                )
        local_success = True
    except asyncpg.exceptions.PostgresError as e:
...
async def insert_data(
    table_name: str, request: Request, records_data: List[Dict[str, Any]] = Body(...), state: ApiState = Depends(typedAppState)
):
    """
...
async def synchronize_all_all_tables(request: Request, state: ApiState = Depends(typedAppState)):
    """
...
async def websocket_data_stream(
    ...
    try:
        ...
        if client_queue in state.websocket_active_clients[table_name]:
            state.websocket_active_clients[table_name].remove(
                client_queue
            )
            logger.info(
