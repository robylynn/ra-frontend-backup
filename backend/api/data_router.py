...
from backend.api.models import ApiState, typedAppState, DatabaseException, DatabaseExceptionReason, DatabaseInsertResult
from backend.utilities.database_utilities import get_mime_type
...
from r2_backend_models.autogenerated.database_models import (
    AutogeneratedPydanticModel,
    DYNAMIC_TABLE_MODELS,
    CapturedImageMap
)
from r2_backend_models.autogenerated.configuration_models import (
    CapturedImage, CapturedImageBatch
)
from backend.api.subscription_manager import _fetch_historical_data
...
async def create_table(
    table_name: str, request: Request, state: ApiState = Depends(typedAppState)
):
    """
    ...
    if hypertable_column and hypertable_column not in columns:
        logger.error(
    ...
    try:
        ...
        async with local_db_pool.acquire() as conn:
            ...
            if hypertable_column:
                ...
                logger.info(f"Table '{table_name}' created successfully on local DB.")
        local_success = True
    except asyncpg.exceptions.PostgresError as e:
    ...
    if state.enable_cloud_db and state.cloud_db_pools:
        cloud_create_tasks = []
        for idx, cloud_db_pool in enumerate(state.cloud_db_pools):
    ...
    message = f"Table '{table_name}' {'and hypertable' if hypertable_column else ''} created successfully on local DB."
...
async def insert_data(
    ...
    state: ApiState = Depends(typedAppState),
):
    """
    ...
    all_cloud_insert_results: Dict[int, List[Dict[str, Any]]] = {
        idx + 1: [] for idx, _ in enumerate(state.cloud_db_pools)
    }
    ...
    if any(not res["success"] for res in local_insert_results):
        ...
        if hasattr(state, "subscription_manager"):
            for record in validated_records_data:
                ...
                await state.subscription_manager.publish(
                    table_name,
                    {
                        "type": "live",
                        "table": table_name,
    ...
    if state.enable_cloud_db and state.cloud_db_pools:
        for cloud_idx, cloud_db_pool in enumerate(state.cloud_db_pools):
...
async def synchronize_all_all_tables(
    ...
    if not state.enable_cloud_db or not state.cloud_db_pools:
        return ApiResponse(
            success=False,
    ...
    local_db_pool: AsyncpgPool = state.local_db_pool
    cloud_db_pools: List[AsyncpgPool] = state.cloud_db_pools
...
async def _insert_into_database(table_name: str, records_data: List[Dict[str, Any]], local_db_pool: AsyncpgPool):
    logger.info(
        f"Attempting to insert batch data into table: {table_name} ({len(records_data)} records)"
    )

    if table_name not in DYNAMIC_TABLE_MODELS:
        error_message = f"Table '{table_name}' not defined in schema.yml or no Pydantic model generated."
        logger.error(
            error_message
        )
        raise DatabaseException(
            message=error_message,
            exception_reason=DatabaseExceptionReason.TABLE_NOT_FOUND
        )
    ...
    if failed_validations:
        error_message = f"Batch insert validation failed for {len(failed_validations)} records in table '{table_name}'."
        logger.error(
            error_message
        )
        raise DatabaseException(
            message=error_message,
            exception_reason=DatabaseExceptionReason.VALIDATION_FAILED,
    ...
    if not validated_records_data:
        raise DatabaseException(
            message=f"No valid records to insert into '{table_name}'.",
            exception_reason=DatabaseExceptionReason.EMPTY_COMMIT
        )

    local_insert_results: List[DatabaseInsertResult] = []
    ...
    if any(not res.success for res in local_insert_results):
        ...
        raise DatabaseException(
            messageg=message,
            exception_reason=DatabaseExceptionReason.COMMIT_PARTIAL_SUCCESS,
            details={
                    "local_results": [r.model_dump() for r in local_insert_results]
                }

        )

@data_router.post("/save_image", response_model=ApiResponse)
async def save_image(request: Request, image: CapturedImage, state: ApiState = Depends(typedAppState)):
    ...
    # images_dir = Path('/home/jetson/r2/ra-frontend/tests/test_images')
    if not images_dir.is_dir():
        error_message = f"Image directory not found: {images_dir}"
    ...
    filename = f"camera_{image.id}_{image.timestamp.strftime('%Y-%m-%d_%H-%M-%S-%f')}.png"
    
    if image.encoding != 'bgr8':
        return ApiResponse(
            message=f"Unknown image encoding {image.encoding}",
            success=False
        )
    
    image_array = np.frombuffer(base64.b64decode(image.image_src), dtype=np.uint8).reshape(image.height, image.width, 3)
    cv2.imwrite(Path(images_dir, filename).as_posix(), image_array)

    try:
        await _insert_into_database(table_name="captured_image_map", records_data=[CapturedImageMap(
            ...
            filename=filename
        ).model_dump()], local_db_pool=state.local_db_pool)
    except Exception as e:
        return ApiResponse(
            success=False,
            message=f"Error saving image to database: {e}"
        )
...
async def websocket_data_stream(
    ...
    if table_name not in state.websocket_broadcast_queues:
        logger.error(
    ...
    state.websocket_active_clients[table_name].append(client_queue)
    logger.info(
        f"WebSocket client {client_id} connected to table '{table_name}'. Active clients: {len(state.websocket_active_clients[table_name])}"
    )
    ...
    try:
        ...
        if client_queue in state.websocket_active_clients[table_name]:
            state.websocket_active_clients[table_name].remove(client_queue)
            logger.info(
