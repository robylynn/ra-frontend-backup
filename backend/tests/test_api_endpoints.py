...
async def test_save_and_get_frontend_config(client: AsyncClient):
    ...
    update_response = await client.post(f"/ui/config/{client_id}", json=updated_config_payload)
    ...
    assert get_updated_response.json()["data"]["config_data"] == updated_config_payload["config_json"]
...
async def test_save_frontend_config_validation_error(client: AsyncClient):
    ...
    invalid_payload = {
        "config_json": "this is not a dict" # Should be a dictionary
    }
...
async def test_create_table(client: AsyncClient):
    ...
    assert response.status_code == 200
    assert response.json()["success"] is True
    assert f"Table '{table_name}' and hypertable created successfully on local DB." in response.json()["message"]
    ...
    assert f"Table '{table_name}' and hypertable created successfully on local DB." in response_again.json()["message"]
...
async def test_insert_data_and_get_data(client: AsyncClient):
    ...
    timestamp_str_2 = (datetime.now(timezone.utc) + timedelta(minutes=1)).isoformat(timespec='milliseconds')
    ...
    # Insert data
    insert_response = await client.post(f"/database/insert_data/{table_name}", json=data_to_insert)
    ...
    # Retrieve data
    get_response = await client.get(f"/database/get_data/{table_name}?limit=1&order_by_column=time")
    assert get_response.status_code == 200
    assert get_response.json()["success"] is True
    retrieved_data = get_response.json()["data"]
    assert len(retrieved_data) == 1
    # Check if the latest record is returned by default or with explicit ordering
    assert retrieved_data[0]["device_id"] == "test-device-02" # Latest by default due to order_by_column="time" DESC
...
async def test_insert_data_validation_error(client: AsyncClient):
    ...
    response = await client.post(f"/database/insert_data/{table_name}", json=invalid_data)
...
async def test_insert_data_unknown_table(client: AsyncClient):
    ...
    response = await client.post("/database/insert_data/non_existent_table_for_insert", json=[{"data": "some_data"}])
    assert response.status_code == 404
    assert response.json()["success"] is False
    assert "not defined in schema.yml" in response.json()["message"]
...
async def test_get_data_unknown_table(client: AsyncClient):
    ...
    assert response.status_code == 404
    assert response.json()["success"] is False
    assert "not defined in schema or models" in response.json()["message"] # Error from _fetch_historical_data
...
async def test_websocket_data_stream_historical(client: AsyncClient):
    ...
    timestamp_str_1 = datetime.now(timezone.utc).isoformat(timespec='milliseconds')
    timestamp_str_2 = (datetime.now(timezone.utc) + timedelta(seconds=1)).isoformat(timespec='milliseconds')
    initial_data = [
        {"activity_time": timestamp_str_1, "user_id": "user1", "action": "login", "duration_ms": 100},
        {"activity_time": timestamp_str_2, "user_id": "user2", "action": "logout", "duration_ms": 200}
    ]
    await client.post(f"/database/insert_data/{table_name}", json=initial_data)

    # Use client.ws_connect for WebSocket tests
    async with client.websocket_connect(f"/database/ws/data/{table_name}?historical_limit=1") as websocket:
        ...
        assert historical_message["data"][0]["user_id"] == "user2" # Latest by default due to order_by_column="time" DESC

        # Insert new data, expect it on WebSocket
        new_timestamp_str = (datetime.now(timezone.utc) + timedelta(seconds=2)).isoformat(timespec='milliseconds')
        new_data = [
            {"activity_time": new_timestamp_str, "user_id": "user3", "action": "view_page", "duration_ms": 300}
        ]
        insert_response = await client.post(f"/database/insert_data/{table_name}", json=new_data)
        assert insert_response.status_code == 201
...
async def test_synchronize_all_tables_disabled_cloud(client: AsyncClient):
    ...
    assert response.status_code == 200
    assert response.json()["success"] is False # Because cloud sync is disabled in 'testing' env
...
async def test_db_query_endpoint(client: AsyncClient):
    ...
    response = await client.get("/status/database/test-query") # CHANGED: Prefix with /status/
    assert response.status_code == 200
    assert response.json()["success"] is True
