...
async def minimal_fastapi_app(setup_minimal_test_db) -> AsyncGenerator[FastAPI, None]:
    ...
    """
    logger.info("Initializing minimal FastAPI app with /health endpoint and DB pool lifespan.")
...
async def test_minimal_app_startup(
    ...
    try:
        ...
        logger.info("SUCCESS: All minimal FastAPI app startup tests passed. DB pool initialized in lifespan, but not queried directly from test.")
    except Exception as e:
