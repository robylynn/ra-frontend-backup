...
SIMULATION_TABLE = os.getenv("SIMULATION_TABLE", "user_activity") # Default table for simulation
INSERT_INTERVAL_SECONDS = int(os.getenv("INSERT_INTERVAL_SECONDS", 5))
SIMULATION_DURATION_SECONDS = int(os.getenv("SIMULATION_DURATION_SECONDS", 60)) # How long the simulation runs

async def insert_batch_data(client: httpx.AsyncClient, table_name: str, num_records: int = 1) -> bool:
    """
    ...
    current_time = datetime.now(timezone.utc).replace(microsecond=0) # Millisecond precision for consistency

    for i in range(num_records):
        record: Dict[str, Any] = {}
        if table_name == "user_activity":
            record = {
                "activity_time": (current_time + timedelta(milliseconds=i)).isoformat(timespec='milliseconds').replace('+00:00', 'Z'),
                "user_id": f"sim_user_{random.randint(1, 100)}",
                "action": random.choice(["login", "logout", "view_page", "click_button", "scroll"]),
                "duration_ms": random.randint(50, 5000)
            }
        elif table_name == "sensor_data":
            record = {
                "time": (current_time + timedelta(milliseconds=i)).isoformat(timespec='milliseconds').replace('+00:00', 'Z'),
            ...
            record = {
                "timestamp": (current_time + timedelta(milliseconds=i)).isoformat(timespec='milliseconds').replace('+00:00', 'Z'),
            ...
            logger.warning(f"Unknown table '{table_name}'. Cannot generate specific data. Skipping insert.")
            return False
        data_to_insert.append(record)

    try:
        response = await client.post(f"/database/insert_data/{table_name}", json=data_to_insert)
        response.raise_for_status()
        logger.info(f"Successfully inserted {len(data_to_insert)} records into '{table_name}'. Status: {response.status_code}")
        return True
    except httpx.HTTPStatusError as e:
        logger.error(f"HTTP error inserting data into '{table_name}': {e.response.status_code} - {e.response.text}")
...
async def get_latest_data(client: httpx.AsyncClient, table_name: str, limit: int = 5):
    ...
    try:
        ...
        data = response.json()
        if data["success"]:
            logger.info(f"Retrieved {len(data['data'])} latest records from '{table_name}':")
            for record in data["data"]:
                logger.info(f"  {record}")
        else:
            logger.warning(f"Failed to retrieve data from '{table_name}': {data['message']}")
    except httpx.HTTPStatusError as e:
        logger.error(f"HTTP error getting data from '{table_name}': {e.response.status_code} - {e.response.text}")
    except httpx.RequestError as e:
...
async def stream_data_via_websocket(api_base_url: str, table_name: str, historical_limit: int = 10):
    """
    ...
    try:
        async with websockets.connect(websocket_url) as websocket:
            ...
            logger.info(f"Received historical data ({len(historical_json.get('data', []))} records): {historical_json['data']}")
        ...
        logger.error(f"WebSocket connection refused. Is the FastAPI server running at {api_base_url}?")
    except Exception as e:
...
async def simulate_ros_stack(api_base_url: str, table_name: str):
    ...
    """
    logger.info(f"Starting ROS stack simulation for table '{table_name}' for {SIMULATION_DURATION_SECONDS} seconds...")
    ...
    async with httpx.AsyncClient(base_url=api_base_url) as http_client:
        ...
        websocket_task = asyncio.create_task(stream_data_via_websocket(api_base_url, table_name))

        start_time = asyncio.get_event_loop().time()
        while asyncio.get_event_loop().time() - start_time < SIMULATION_DURATION_SECONDS:
...
if __name__ == "__main__":
    ...
    logger.info(f"Running simulation with API_BASE_URL={API_BASE_URL}, SIMULATION_TABLE={SIMULATION_TABLE}")
