...
async def run_websocket_test(api_base_url: str, table_name: str, historical_limit: int):
    ...
    test_user_id_prefix = "ws-test-user-" + datetime.now().strftime("%Y%m%d%H%M%S") # Unique ID for each test run
    ...
    initial_activity_time = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(seconds=1)
    initial_activity_time_iso = initial_activity_time.isoformat(timespec='milliseconds').replace('+00:00', 'Z')
    ...
    live_activity_time = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(seconds=2)
    live_activity_time_iso = live_activity_time.isoformat(timespec='milliseconds').replace('+00:00', 'Z')
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            insert_response_initial = await client.post(http_insert_url, json=initial_data_to_insert)
            insert_response_initial.raise_for_status() # Raise an exception for HTTP errors
            logger.info(f"Initial data inserted via HTTP: {insert_response_initial.json()}")
        ...
        async with websockets.connect(websocket_url) as websocket:
            ...
            historical_json = json.loads(historical_message) # Parse JSON string to Python dict
            
            logger.info(f"Received historical data: {historical_json}")
            
            assert historical_json["type"] == "historical", "Expected historical data message type."
            assert isinstance(historical_json["data"], list), "Historical data should be a list."
            
            # Now, we expect to get the pre-inserted data as historical
            assert len(historical_json["data"]) == 1, f"Expected 1 historical record, got {len(historical_json['data'])}"
            
            # Validate the content of the historical record
            received_historical_record = historical_json["data"][0]
            assert received_historical_record["user_id"] == initial_data_to_insert[0]["user_id"]

            # Convert received ISO string back to datetime for robust comparison
            received_historical_time = datetime.fromisoformat(received_historical_record["activity_time"].replace('Z', '+00:00'))
            expected_historical_time = datetime.fromisoformat(initial_data_to_insert[0]["activity_time"].replace('Z', '+00:00'))
            
            # Assert that the received historical time matches the expected, ignoring microseconds beyond milliseconds
            assert abs((received_historical_time - expected_historical_time).total_seconds()) < 0.001, \
                f"Historical activity_time mismatch: Expected {expected_historical_time}, got {received_historical_time}"

            logger.info(f"Successfully received {len(historical_json['data'])} historical records.")

            # 2. Insert new data to trigger a live WebSocket update
            logger.info("Inserting new data via HTTP to trigger live WebSocket update...")
            async with httpx.AsyncClient() as client:
                insert_response_live = await client.post(http_insert_url, json=live_data_to_insert)
            ...
            assert received_live_record["user_id"] == expected_live_record["user_id"], \
                f"Live data user_id mismatch: Expected {expected_live_record['user_id']}, got {received_live_record['user_id']}"
            assert received_live_record["action"] == expected_live_record["action"], \
                f"Live data action mismatch: Expected {expected_live_record['action']}, got {received_live_record['action']}"
            
            # Convert received ISO string back to datetime for robust comparison
            received_live_time = datetime.fromisoformat(received_live_record["activity_time"].replace('Z', '+00:00'))
            expected_live_time = datetime.fromisoformat(expected_live_record["activity_time"].replace('Z', '+00:00'))

            # Assert that the received live time matches the expected, ignoring microseconds beyond milliseconds
            assert abs((received_live_time - expected_live_time).total_seconds()) < 0.001, \
        ...
        logger.error(f"HTTP error during data insertion: {e.response.status_code} - {e.response.text}")
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python ws_test_client.py <api_base_url> <table_name> <historical_limit>")
        sys.exit(1)
