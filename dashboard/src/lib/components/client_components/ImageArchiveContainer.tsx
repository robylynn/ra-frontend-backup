...
const ImageArchiveContainer = ({
    ...
    const [selectedImage, setSelectedImage] = useState<capturedImageType | null>(
        null
    );
    ...
    useEffect(() => {
        const getDefectImages = async () => {
            logMessage(`image limit is ${maxImages}`, 'info');
            const image_batch =
                await fetchFromBackendApi<capturedImageBatchType>(
                    `/api/backend/database/captured_images?limit=${maxImages}`,
                    'GET',
                    undefined,
                    capturedImageBatch
                );
    ...
    const filteredImages = capturedImages.images.filter(
        ...
        <div className={`w-full h-full flex flex-col ${className}`}>
            ...
            {showFilters && (
                <div className="p-2 bg-gray-800 border-b border-gray-600">
                    ...
                    <div className="text-xs text-gray-400">
                        {filteredImages.length} of {capturedImages.images.length} images
                    </div>
                </div>
            )}
            ...
            <div className="flex-1 p-2 overflow-y-auto">
                <div className={`grid ${gridCols} gap-1`}>
                    {capturedImages.images.map((image) => (
                        <div
                            key={image.id}
