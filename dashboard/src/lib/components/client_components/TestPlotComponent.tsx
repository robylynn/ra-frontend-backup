...
export const TestPlotComponent: React.FC<PlotProperties> = (
    ...
    const [websocketDataState, setWebsocketDataState] = useState<'connected' | 'waiting' | 'invalid_data'>('waiting');
    ...
    const websocketMessageCallback = (message: WebSocketMessage) => {
        ...
        // let validatedData;
        try {
            const validatedData = validateWebsocketData<sensorDataBatch>(
                message,
                sensorDataBatchSchema
            );
            ...
            showAlert(`Unvalidated websocket data: ${error.message}`, 'error');
    ...
    return (
        <div>
            ...
            {chartData.length > 0 ? (
                ...
                <LoadingIndicator text={websocketDataState === 'waiting' ? `WAITING FOR WEBSOCKET` : `WAITING FOR VALID DATA`} />
            )}
        </div>
    );
};
