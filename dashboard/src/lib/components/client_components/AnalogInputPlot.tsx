...
import {
  ROSAnalogIOStateInterface,
  DatabaseROSAnalogIOStateArray,
...
const AnalogInputPlot = (props: { length: number }) => {
  ...
  useEffect(() => {
    const get_initial_data = async () => {
      await timeoutFetch<Array<ROSAnalogIOStateInterface>>(
  ...
  // let a = 5;
  let configured_IO_points = IOPoints.getConfiguredIOPoints(
