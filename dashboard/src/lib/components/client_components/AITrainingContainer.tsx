...
export const AITrainingContainer = ({
    ...
    const fetchProjectsFromROS = async () => {
        ...
        try {
            if (!dashboardContext.ra_ros_websocket?.isConnected) {
                ...
                alert(
                    'ROS WebSocket is not connected. Please check your connection.'
                );
                setIsFetchingProjects(false);
                return;
            }

            if (!dashboardContext.ros_services?.ai_training_services) {
            ...
            if (
                typeof dashboardContext.ros_services?.ai_training_services.get_projects !==
                'function'
            ) {
                ...
                console.log(
                    ...
                    Object.getOwnPropertyNames(
                        Object.getPrototypeOf(
                            dashboardContext.ros_services?.ai_training_services
                        )
                    )
                );
            ...
            dashboardContext.ros_services?.ai_training_services.get_projects(
                (results) => {
                    try {
                        if (results.success && results.projects) {
                            ...
                            console.warn(
                                'No projects data in ROS response or unsuccessful'
                            );
                            setProjects([]);
                            setIsFetchingProjects(false);
                        }
                ...
                () => {
                    ...
                    alert(
                        'Failed to get projects from ROS. Check console for details.'
                    );
                    setProjects([]);
                    setIsFetchingProjects(false);
                }
            ...
            console.error('Error details:', {
                ...
                aiTrainingServices: dashboardContext.ros_services?.ai_training_services,
            });
    ...
    const startTraining = async () => {
        if (!selectedTrainingProject || !selectedValidationProject) {
            console.error(
                'Both training and validation projects must be selected'
            );
            return;
        }
        ...
        try {
            dashboardContext.ros_services?.ai_training_services.start_ai_training(
                  ...
                  : "❌ Disconnected"}
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-3">
                            <div>
                                ...
                                >
                                    <option value="yolo_nas_s">
                                        YOLO NAS S
                                    </option>
                                    <option value="yolo_nas_m">
                                        YOLO NAS M
                                    </option>
                                    <option value="yolo_nas_l">
                                        YOLO NAS L
                                    </option>
                                </select>
                            </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex gap-1">
                                <button
                                    onClick={() =>
                                        window.open('/labelstudio', '_blank')
                                    }
                        ...
                        {selectedTrainingProject ===
                            ...
                            selectedTrainingProject && (
                                <div className="mt-1 p-1 bg-red-900/30 border border-red-600 rounded text-xs text-red-400">
                                    ⚠️ Training and validation datasets must be
                                    different
                                </div>
                            )}
                        ...
                        {projects.length === 0 && !isFetchingProjects && (
                            <div className="mt-1 p-2 bg-yellow-900/30 border border-yellow-600 rounded text-xs text-yellow-400">
                                ⚠️ No projects loaded. Click "📂 Get Projects"
                                to fetch from ROS.
                            </div>
                        )}
                        ...
                        {!dashboardContext.ra_ros_websocket?.isConnected && (
                            <div className="mt-1 p-2 bg-red-900/30 border border-red-600 rounded text-xs text-red-400">
                                ❌ ROS connection required to fetch projects and
                                start training.
                            </div>
                        )}
                    </div>
                        ...
                        <div className="h-full overflow-y-auto space-y-1">
                            {trainingJobs.length === 0 ? (
                                <div className="text-gray-400 text-xs p-2 text-center">
                                    No training jobs yet. Start training to see
                                    jobs here.
                                </div>
                            ) : (
                                trainingJobs.map((job) => (
                                    ...
                                    >
                                        ...
                                        <div className="text-gray-400 text-xs mb-1">
                                            Train: #{job.train_project_id} •
                                            Val: #{job.val_project_id}
                                        </div>

                                        <div className="text-gray-400 text-xs mb-2">
                                            {job.model_type} • {job.epochs}{' '}
                                            epochs • batch {job.batch_size} •{' '}
                                            {new Date(
                                                job.created_at
                                            ).toLocaleDateString()}
                                        </div>
