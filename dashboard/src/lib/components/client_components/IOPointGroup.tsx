...
const IOPointGroup = ({
    ...
    setLoading,
    setErrorMessage,
}: IOPointGroupInterface) => {
    const { IOPoints, setIOPoints } = useContext(IOPointContext);
    ...
    useEffect(() => {
        if (dashboardContext.hardware_configuration) {
            if (!IOPoints.initialized) {
                let config =
                    dashboardContext.hardware_configuration.io_system.copy();
                config.assignUUIDs();
                setIOPoints({
    ...
    useEffect(() => {
        if (dashboardContext.getGpioConfigurationState(point_type)) {
            setIOPoints({
                payload: {
    ...
    const handleDragEnd = (result: DropResult) => {
        ...
        const destinationPointChannel =
            IOPoints.getConfiguredIOPoints(point_type)[result.destination.index]
                .channel;
        const sourcePointChannel =
            IOPoints.getConfiguredIOPoints(point_type)[result.source.index]
                .channel;
        setIOPoints({
            payload: {
    ...
    return (
        <>
            {/* // DEBUG ONLY, REMOVE */}
            {/* <p>{`${dashboardContext.digital_out_data?.values}`}</p>
            <button
                onClick={() =>
            ...
            <p>{`${dashboardContext.analog_out_data?.values}`}</p>
            <button
                onClick={() =>
