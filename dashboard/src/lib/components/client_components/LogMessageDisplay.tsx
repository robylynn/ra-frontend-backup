...
export function LogViewerIcon() {
    ...
    const toggleOverlay = () => {
        setIsOpen((prev) => !prev);
        if (!isOpen) {
            // If opening the overlay, reset count
    ...
    useEffect(() => {
        if (isOpen) {
            // Only scroll to bottom if overlay is open
            scrollToBottom();
        }
    }, [allLogs, isOpen]);
    ...
    useEffect(() => {
        ...
        const unRegisterListener = registerMessageListener(
            listenerId,
            callback,
            { types: ['status', 'error', 'live', 'historical'], table: 'logs' }
        );
    ...
    const renderLogMessage = (log: WebSocketMessage, index: number) => {
        ...
        switch (log.type) {
                ...
                content = (
                    <>
                        <span className="font-bold">
                            [{timestamp}] HISTORICAL [{log.table}]:
                        </span>
            ...
            case 'status':
                ...
                content = (
                    <>
                        <span className="font-bold">[{timestamp}] STATUS:</span>{' '}
                        {log.message || `Table ${log.table} is ${log.status}.`}
                    </>
                );
                break;
            case 'error':
                ...
                content = (
                    <>
                        <span className="font-bold">[{timestamp}] ERROR:</span>{' '}
                        {log.message}
                    </>
                );
                break;
