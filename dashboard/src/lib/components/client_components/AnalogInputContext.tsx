...
export const AnalogInputContext =
  createContext<AnalogInputContextInterface>(null);
...
export const AnalogInputProvider = ({ children }) => {
  ...
  const addInput = (input: IOPointConfiguration) => {
    ...
    const availableChannel = [0, 1, 2, 3].find(
  ...
  return (
    <AnalogInputContext.Provider
      value={{ inputs, setInputs, addInput, updateInput, deleteInput }}
    >
      {children}
    </AnalogInputContext.Provider>
  );
