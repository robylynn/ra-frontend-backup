...
const IOPlots = (props: {
    ...
    useEffect(() => {
        ...
        initialDataAcquired.current = dashboardContext.configuration.io_plots[
            props.point_type
        ].reduce(
    ...
    useEffect(() => {
        dashboardContext.configuration.io_plots[props.point_type].forEach(
            (plot_configuration, plot_index) => {
                if (
                    !Object.keys(completeIOData).includes(plot_index.toString())
                ) {
                    setCompleteIOData((d) => ({
                        ...d,
                        [plot_index]: [],
    ...
    useEffect(() => {
        const get_initial_data = async (
            ...
            let initial_data = new Promise<IOFetchDataInterface>(
            ...
            await timeoutFetch<Array<ROSIOStateInterface>>(
                ...
                .then((data?) => {
                    if (data) {
                        ...
                        console.log(
                            `Got ${length} database documents for ${IOPointType[props.point_type]}`
                        );

                        const initial_chart_data = data_documents.documents.map(
                            (d) => {
                                ...
                                for (const key in d.values) {
                                    if (
                                        ...
                                        data_point[parseInt(key)] =
                                            d.values[key];
                                    else
                                        data_point[parseInt(key)] = d.values[
                                            key
                                        ]
                                            ? 1
                                            : 0;
                        ...
                        initial_data = new Promise<IOFetchDataInterface>(
                            (resolve, reject) =>
                                resolve(initial_chart_data.reverse())
                        );
                    }
                })
                .catch((e) => {
    ...
    const configured_enabled_IO_points =
        IOPoints.getConfiguredAndEnabledIOPoints(props.point_type);
    ...
    const parseIOData = (data: PlotDataPoint[]): PlotInputData => {
                        ...
                        return {
                            ...
                            data: data.map(
                                (data_point) =>
                                    ({
                                        [numeric_point_index]:
                                            data_point[numeric_point_index],
            ...
            {!Object.keys(completeIOData) ? (
                ...
                <div>
                    <div key={'plots'}>
                        {dashboardContext.configuration.io_plots[
                            props.point_type
                        ].map((plot_configuration, plot_index) => {
                            let labels: Record<
                                number,
                                { data_name: string; y_label: string }
                            > = {};

                            plot_configuration.data_sources.forEach(
                                (source) => {
                                    const point = IOPoints.getIOPoints(
                                        props.point_type
                                    )?.[source];
                                ...
                                <DataPlot
                                    ...
                                    available_sources={configured_enabled_IO_points.filter(
                                        (point) =>
                                            !dashboardContext.configuration.io_plots[
                                                props.point_type
                                            ][plot_index].data_sources.includes(
                                                point.channel
                                            )
                                    )}
                                    ...
                                    add_trace_callback={(
                                        ...
                                        setDashboardContext({
                                            payload: {
                                                ...
                                                data_sources: [
                                                    ...dashboardContext
                                                        .configuration.io_plots[
                                                        props.point_type
                                                    ][plot_index].data_sources,
                                                    io_channel,
                                                ],
                                            },
