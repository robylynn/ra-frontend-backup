...
export const RosWebsocket: React.FC<WebsocketProps> = ({
  ...
  const configureSubscriptions = () => {
    ...
    addSubscription({
      ...
      callback: (message) => {
        setDashboardContext({
          payload: {
            analog_out_data: message as IRosTypeR2CInterfacesAnalogOutData,
          },
    ...
    addSubscription({
      ...
      callback: (message) => {
        ...
        setDashboardContext({
          payload: {
            digital_out_data: message as IRosTypeR2CInterfacesDigitalOutData,
          },
  ...
  const connectRosbridge = useCallback(() => {
    ...
    logMessage(`Attempting to connect to Rosbridge: ${websocketUrl}`, "debug");
    ...
    newRos.on("connection", () => {
      ...
      if (isMounted.current) {
        ...
        timeoutServiceCall(nodes_service, {}, reconnectInterval / 2)
          .then(
            (nodes) => {
              logMessage(`Found ROS nodes: ${JSON.stringify(nodes)}`, "info");
              return true;
            },
            (error) => {
              logMessage(
    ...
    newRos.on("error", (rosError) => {
      ...
      if (isMounted.current) {
        ...
        logMessage(`ROSLIB error: ${JSON.stringify(rosError)}`, "error");
