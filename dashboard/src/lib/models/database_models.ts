...
export class DatabaseDocumentIOPoint
    implements DatabaseDocumentIOPointInterface
{
    configured: boolean;
    state?: number | boolean;
...
export class DatabaseIOState
    ...
    implements DatabaseIOStateInterface
{
    ...
    constructor(document?: DatabaseIOStateInterface) {
        super(document);
        if (document != undefined) {
            this.digital_inputs = new DatabaseDocumentIOPort(
                document.digital_inputs
            );
            this.digital_outputs = new DatabaseDocumentIOPort(
                document.digital_outputs
            );
        }
    }
}
...
export class ROSIOState
    extends DatabaseDocument
    implements ROSIOStateInterface
{
    values: Record<string, number> | Array<boolean>;
    stamp: ROSTimestamp;
    time_sec: number;
...
export interface ROSAxisStateInterface
    extends DatabaseDocumentInterface,
...
export class ROSAxisState
    extends DatabaseDocument
    implements ROSAxisStateInterface
{
    stamp: ROSTimestamp;
    velocity: number;
    position: number;
