...
import { z } from 'zod';
import { apiResponse, apiResponseType } from '@/lib/models/autogenerated/system_models';
...
// }
export const backendApiResponseBaseSchema = z.object({
    authenticated: z.boolean(),
    backend_response: apiResponse.extend({
        data: z.any().optional(),
    }).optional(),
    proxy_error: z.boolean().optional(),
    proxy_error_string: z.string().optional(),
});
export type backendApiResponseSchemaType = z.infer<typeof backendApiResponseBaseSchema>;
...
//     backend_response?: apiResponseType;
//     proxy_error?: boolean;
//     proxy_error_string?: string;
// }

export function createAPIResponse(res: backendApiResponseSchemaType) {
    return backendApiResponseBaseSchema.parse(res);
...
export const createApiResponseSchema = <T>(schema?: z.ZodSchema<T>) => {
    ...
    if (schema)
        return backendApiResponseBaseSchema.extend({
            backend_response: apiResponse.extend({
                data: schema.nullable(),
            }),
        });
    else
        return backendApiResponseBaseSchema;    
