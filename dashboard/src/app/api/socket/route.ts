...
export async function SOCKET(
  ...
  if (sessionCookieValue) {
    ...
    try {
      ...
      if (Date.now() >= authenticatedPayload.exp * 1000) {
        console.error("Websocket connection rejected due to expired token.");
      ...
      if (decodeError instanceof Error) {
        console.error("[WS Debug] Decode Error Message:", decodeError.message);
      }
  ...
  if (!authenticatedPayload) {
    console.warn("[WS] Unauthorized WebSocket connection. Closing connection.");
